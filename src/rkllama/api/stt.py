...
class STTModelRKNN:
    ...
    def __call__(self, x: np.ndarray, chunk_len: int):
        ...
        logger.debug("Original input shape received", shape=x.shape)
        # Padding to match RKNN model input
        padded_x = np.zeros((x.shape[0], chunk_len), dtype=np.float32)
        padded_x[:, :x.shape[1]] = x
        logger.debug("Padded input shape", shape=padded_x.shape)

        # RUn RKNN inference
        logits = self.model_rknn.inference(inputs=[padded_x], data_format="nchw")[0]
        logger.debug("Logits generated by RKNN", shape=logits.shape)
        ...
        logger.debug("Logits after trim", shape=logits.shape)
    ...
    def load_audio_from_bytes(self, data: bytes) -> np.ndarray:
        ...
        logger.debug("Original SR from audio file", sample_rate=sr)
        ...
        if sr != SAMPLE_RATE:
            logger.debug("Resample audio file", target_sample_rate=SAMPLE_RATE)
    ...
    def split_audio(self, samples, overlap_sec=1.0):
        ...
        logger.debug("Chunk size determined", chunk_sec=chunk_sec)
        ...
        logger.debug("Audio split parameters", chunk_len=chunk_len, overlap=overlap, step=step, samples=len(samples))
        ...
        # Return the chunks
        logger.debug("Audio chunks generated", num_chunks=len(chunks))
